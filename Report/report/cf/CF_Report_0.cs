namespace Report
{
    ...
    /// <summary>
    /// 统计--采伐统计表--皆伐伐区调查设计汇总表
    /// </summary>
    internal class CF_Report_0 : ReportEntity
    {
        protected List<ZT_CF_Mid> m_dataList;

        private DataTable m_table;
        /// <summary>
        /// 统计--采伐统计表--皆伐伐区调查设计汇总表：构造器
        /// </summary>
        public CF_Report_0()
        {
            base.ReportID = "0";
        ...
        public override void InitialReport(ReportParamter pReportParamter)
        {
            ...
            //需要获取的字段
            string subFields = "SHI,XIAN,XIANG,CUN,XIAO_BAN,LIN_ZHONG,YOU_SHI_SZ,LIN_BAN,QI_YUAN,PINGJUN_NL,YU_BI_DU,PINGJUN_SG,PINGJUN_XJ,HUO_LMGQXJ,GXFS,CFMJ,CFXJ,CFZS,CCLV,MIAN_JI,DI_LEI,GXDJ,SOUTH,ZXJ";////"+MIAN_JI,DI_LEI,AJLX,XWZT,SSLDMJ,SSLMXJ,SSMZZS,SSYSMMZS,FSRQ,JLRQ"
        ...
        private void MakeRow(IGrouping<string, ZT_CF_Mid> data, string xianName, string xiangName, string cunName, string linban, string xiaoban, string linzhong, string shuzhong, string qiyaun, string linling, string dengxingdengji)
        {
            ...
            double linling1 = data.Where(p => Convert.ToInt32(p.CFFS) == 11).Sum(p => p.PINGJUN_NL);
            row["LINLING"] = linling1;
            double yubidu1 = data.Where(p => Convert.ToInt32(p.CFFS) == 11).Sum(p => p.YU_BI_DU);
            row["YUBIDU"] = yubidu1;
            ...
            double gqxj = data.Where(p => Convert.ToInt32(p.CFFS) == 11).Sum(p => p.HUO_LMGQXJ);
            row["GQXJ"] = gqxj;
            double cfmj = data.Where(p => Convert.ToInt32(p.CFFS) == 11).Sum(p => p.CFMJ);
            row["CFMJ"] = cfmj;
            double cfxj = data.Where(p => Convert.ToInt32(p.CFFS) == 11).Sum(p => p.CFXJ);
            row["CFXJ"] = cfxj;
            double cfzs = data.Where(p => Convert.ToInt32(p.CFFS) == 11).Sum(p => p.CFZS);
            row["CFZS"] = cfzs;
            double cfcc = data.Where(p => Convert.ToInt32(p.CFFS) == 11).Sum(p => p.CCLV);
            row["CFCC"] = cfcc;
        ...
        protected override DataTable MakeTable()
        {
            ...
            table.Columns.Add("GDGXFS", typeof(double));

            table.Columns.Add("GDGXSZ", typeof(double));
            table.Columns.Add("GDGXSJ", typeof(double));
