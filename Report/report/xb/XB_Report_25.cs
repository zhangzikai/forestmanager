...
{
    /// <summary>
    /// 小班报表25   森林经营类型按立地质量统计表
    /// </summary>
    public class XB_Report_25 : XB_Report_Base
    {
        public XB_Report_25()
        {
        ...
        protected void MakeRow(IEnumerable<FOR_XIAOBAN_Mid> data, string xzName, string tdsyq)
        {
            ...
            double L1mj = data.Where(p => p.JKZK == "1").Sum(p => p.YXMJ);
            row["L1mj"] = L1mj;
            double L1xj = data.Where(p => p.JKZK == "1").Sum(p => p.ZXJ);
            row["L1xj"] = L1xj;

            double L2mj = data.Where(p => p.JKZK == "2").Sum(p => p.YXMJ);
            row["L2mj"] = L2mj;
            double L2xj = data.Where(p => p.JKZK == "2").Sum(p => p.ZXJ);
            row["L2xj"] = L2xj;

            double L3mj = data.Where(p => p.JKZK == "3").Sum(p => p.YXMJ);
            row["L3mj"] = L3mj;
            double L3xj = data.Where(p => p.JKZK == "3").Sum(p => p.ZXJ);
            row["L3xj"] = L3xj;

            double L4mj = data.Where(p => p.JKZK == "4").Sum(p => p.YXMJ);
            row["L4mj"] = L4mj;
            double L4xj = data.Where(p => p.JKZK == "5").Sum(p => p.ZXJ);
            row["L4xj"] = L4xj;

            row["hjmj"] = L1mj + L2mj + L3mj + L4mj;
            row["hjxj"] = L1xj + L2xj + L3xj + L4xj;
