...
{
    public class XB_Report_xbyzpg : XB_Report_Base
    {
        public XB_Report_xbyzpg()
        {
            //  乔木林单位面积蓄积变化统计表
            SheetName = "小班评估因子表";
            m_firstRow = 8;
        ...
        protected void MakeRow(IEnumerable<FOR_XIAOBAN_Mid> data, string xian, string xiang, string cun)
        {
            foreach (var mid in data)
            {
                ...
                row["tdsyq"] = MDM.FindName("TDJYQ", mid.LD_QS);
                ...
                row["jyfs"] = mid.Remarks;
                row["fcfs"] = mid.Remarks;
                row["fcbl"] = 40;
        ...
        protected override DataTable MakeTable()
        {
            ...
            table.Columns.Add("jyfs", typeof(string));
            table.Columns.Add("fcfs", typeof(string));
            table.Columns.Add("fcbl", typeof(double));
            m_table = table;
            return m_table;
        }
    }
