...
namespace td.forest.report.xb
{
    ...
    /// 小班报表1-1:各类林按林种面积统计表
    /// </summary>
    public class XB_Report_1_1 : XB_Report_Base
    {
        public XB_Report_1_1()
        {
            SheetName = "1-1";
            m_firstRow = 7;
            m_firstColumn = 1;
        ...
        public override List<FOR_XIAOBAN_Mid> InitialReport(FindMidFromLayer<FOR_XIAOBAN_Mid> midLayer)
        {
            //需要获取的字段
            m_midLayer = midLayer;
            string subFields = "SHI,XIAN,XIANG,CUN,TDJYQ,LD_QS,DI_LEI,SEN_LIN_LB,LIN_ZHONG,YXMJ";
        ...
        public override DataTable FromMid2Table()
        {
            ...
            var sllb = m_xb_dataList.Where(p => p.SEN_LIN_LB.Length > 0);
            var linZh = m_xb_dataList.Where(p => p.LIN_ZHONG.Length > 0);
            if (linZh.Count() < 1) { return m_table; }
            //本月整体统计
            string xianName = MDM.FindXQName(m_xb_dataList[0].SHI);
            MakeRowByQS(linZh, xianName);
            //按乡统计
            var xiangGrps = linZh.GroupBy(p => p.XIAN);
        ...
        private void MakeRowByQS(IEnumerable<FOR_XIAOBAN_Mid> data, string xzName)
        {
            ...
            {
                string sllb = MDM.FindName("SEN_LIN_LB", xg.Key);
                var linZhGrp=xg.GroupBy(p => p.LIN_ZHONG);
                foreach (var linZh in linZhGrp)
                {
                    string lz = MDM.FindName("LIN_ZHONG", linZh.Key);
                    MakeRow(linZh, xzName, sllb, lz);
                }
            }
        }
        protected void MakeRow(IEnumerable<FOR_XIAOBAN_Mid> data, string xzName, string sllb,string linzh)
        {
        ...
        protected override DataTable MakeTable()
        {
            ...
            table.Columns.Add("SLLB", typeof(string));
            table.Columns.Add("LZ", typeof(string));
