...
BEGIN
	BEGIN TRY
		...
		EXEC('CREATE VIEW T_STAT_XZ AS 
			SELECT SHENG, SUBSTRING(XIAN,3,2) AS SHI, XIAN, XIANG, CUN, LIN_YE_JU ,LIN_CHANG ,LIN_BAN ,case when MIAN_JI is NULL then 0 else ROUND(MIAN_JI,2) end AS MIAN_JI,DI_MAO,
			(case
		...
		EXEC('CREATE VIEW T_STAT_BH AS 
			SELECT SHENG, SUBSTRING(XIAN,3,2) AS SHI, XIAN, XIANG, CUN, LIN_YE_JU ,LIN_CHANG ,LIN_BAN , BHYY, case when MIAN_JI is NULL then 0 else ROUND(MIAN_JI,2) end AS MIAN_JI,BH_DJ,Q_BH_DJ,
			(case
