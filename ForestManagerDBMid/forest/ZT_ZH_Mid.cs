...
{
    [TDDBRecord(Table = "ZT_ZH")]
    public class ZT_ZH_Mid
    {
        ...
        //字段别名:有害生物类型
        [TDDBProperty(Column = "YHSW", Index = 31)]
        public string YHSW { get; set; }
        //字段别名:死树株数
        [TDDBProperty(Column = "SSZS", Index = 32)]
        public int SSZS { get; set; }
        //字段别名:发现日期
        [TDDBProperty(Column = "FXRQ", Index = 33)]
        public string FXRQ { get; set; }
        //字段别名:记录日期
        [TDDBProperty(Column = "JLRQ", Index = 34)]
        public string JLRQ { get; set; }
        //字段别名:成灾等级
        [TDDBProperty(Column = "CZDJ", Index = 35)]
        public string CZDJ { get; set; }
        //字段别名:损失面积
        [TDDBProperty(Column = "SSMJ", Index = 36)]
        ...
        public int FXMongth
        {
            get
            {
                if(FXRQ.Length==8){return Convert.ToInt32(FXRQ.Substring(4,2));}
                return 1;
            }
        }
        public int FXYear
        {
            get
            {
                if (FXRQ.Length == 8) { return Convert.ToInt32(FXRQ.Substring(0, 4)); }
                return 2016;
            }
        }
    }
