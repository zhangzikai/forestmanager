namespace TopologyCheck.Checker
{
    ...
    internal class TopoClassChecker : ITopologyChecker
    {
        ...
        public TopoClassChecker(IList<IFeatureClass> pList)
        {
            ...
            if (this.m_TempPath == "")
            {
                this.m_TempPath = UtilFactory.GetConfigOpt().GetRootPath() + @"\" + UtilFactory.GetConfigOpt().GetConfigValue("TempPath");
            }
            this.m_FCList = pList;
        }
        ...
        private object CheckFeatureGap(IFeature pFeature, IFeatureClass pFClass)
        {
            IGeometry shapeCopy = pFeature.ShapeCopy;
