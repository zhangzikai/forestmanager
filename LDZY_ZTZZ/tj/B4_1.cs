...
namespace LDZY_ZTZZ.tj
{
    public class B4_1 : ZDZYTJBase
    {
        ...
        protected override void MakeRow(IEnumerable<ZT_LDZZ_2016_Mid> data, string key, string xiban, string tudiquanshu, string qiyuan, string linzhong, string shuzhong, string ldlx, string lmqs, string bz)
        {
            ...
            double mpd = data.Where(p => Convert.ToString(p.LDLX) == "6").Sum(p => p.MIAN_JI);
            double zmj = data.Sum(p => p.MIAN_JI);

            double zxj = data.Sum(p => p.SLXJ);

            num2 = zmj;
            num3 = zxj;

            num4 = data.Where(p => Convert.ToString(p.LING_ZU) == "1").Sum(p => p.MIAN_JI);
            num5 = data.Where(p => Convert.ToString(p.LING_ZU) == "1").Sum(p => p.SLXJ);

            num6 = data.Where(p => Convert.ToString(p.LING_ZU) == "2").Sum(p => p.MIAN_JI);

            num7 = data.Where(p => Convert.ToString(p.LING_ZU) == "2").Sum(p => p.SLXJ);
            num8 = data.Where(p => Convert.ToString(p.LING_ZU) == "3").Sum(p => p.MIAN_JI);
            num9 = data.Where(p => Convert.ToString(p.LING_ZU) == "3").Sum(p => p.SLXJ);
            num10 = data.Where(p => Convert.ToString(p.LING_ZU) == "4").Sum(p => p.MIAN_JI);

            num11 = data.Where(p => Convert.ToString(p.LING_ZU) == "4").Sum(p => p.SLXJ); ;

            num12 = data.Where(p => Convert.ToString(p.LING_ZU) == "5").Sum(p => p.MIAN_JI);
            num13 = data.Where(p => Convert.ToString(p.LING_ZU) == "5").Sum(p => p.SLXJ);
