...
namespace LDZY_ZTZZ.tj
{
    public class ZZDJ : ZDZYTJBase
    {
        ...
        protected override void MakeRow(IEnumerable<ZT_LDZZ_2016_Mid> data, string key, string xiban, string tudiquanshu, string qiyuan, string linzhong, string shuzhong, string ldlx, string lmqs, string bz)
        {
            ...
            num4 = data.Where(p => p.ZL_DJ == "1" && p.XI_BAN == xiabn_tq).Sum(p => p.MIAN_JI);
            num5 = data.Where(p => p.ZL_DJ == "1" && p.XI_BAN == xiabn_tq).Sum(p => p.SLXJ);
            num6 = data.Where(p => p.ZL_DJ == "2" && p.XI_BAN == xiabn_tq).Sum(p => p.MIAN_JI);
            num7 = data.Where(p => p.ZL_DJ == "2" && p.XI_BAN == xiabn_tq).Sum(p => p.SLXJ);

            num8 = data.Where(p => p.ZL_DJ == "3" && p.XI_BAN == xiabn_tq).Sum(p => p.MIAN_JI);
            num9 = data.Where(p => p.ZL_DJ == "3" && p.XI_BAN == xiabn_tq).Sum(p => p.SLXJ);
            num10 = data.Where(p => p.ZL_DJ == "4" && p.XI_BAN == xiabn_tq).Sum(p => p.MIAN_JI);
            num11 = data.Where(p => p.ZL_DJ == "4" && p.XI_BAN == xiabn_tq).Sum(p => p.SLXJ);
            num12 = data.Where(p => p.ZL_DJ == "5" && p.XI_BAN == xiabn_tq).Sum(p => p.MIAN_JI);
            num13 = data.Where(p => p.ZL_DJ == "5" && p.XI_BAN == xiabn_tq).Sum(p => p.SLXJ);
