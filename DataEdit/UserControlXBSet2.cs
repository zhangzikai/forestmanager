namespace DataEdit
{
    ...
    public class UserControlXBSet2 : UserControlBase1
    {
        ...
        private void DoAutoInput()
        {
            ...
            try
            {
                ...
                editWorkspace.StartEditing(false);
                int num = 0;
                for (int i = 0; i < this.tList.Nodes.Count; i++)
                {
                    IGeometry shapeCopy;
                    IGeometry geometry7;
                    Application.DoEvents();
                    num++;
                    node = this.tList.Nodes[i];
                    ...
                    if (!pFeature.Shape.IsEmpty)
                    {
                        ...
                        shapeCopy = GISFunFactory.UnitFun.ConvertPoject(shapeCopy, this.mHookHelper.FocusMap.SpatialReference);
                        this.mHookHelper.ActiveView.PartialRefresh(esriViewDrawPhase.esriViewGeography, this.m_EditLayer, shapeCopy.Envelope);
                    }
                    IFeature feature = null;
                    int index = pFeature.Fields.FindField("DT_SRC");
                    int num4 = pFeature.Fields.FindField("BHYY");
                    int num5 = pFeature.Fields.FindField("GXSJ");
                    string[] strArray = node.GetDisplayText(5).Split(new char[] { ',' });
                    bool flag = true;
                    object[] objArray = new object[] { "处理相交数据共 ", this.tList.Nodes.Count, "个 , 第 ", (i + 1).ToString(), " 个" };
                    ...
                    for (int j = 1; j < tag.Count; j++)
                    {
                        ...
                        IGeometry geometry2;
                        IFeature feature4;
                        IFeature psf = tag[j] as IFeature;
                        if (double.Parse(strArray[j - 1]) >= 95.0)
                        {
                            if (pFeature.get_Value(index).ToString().Trim() == "99")
                            {
                                if ((pFeature.get_Value(index).ToString().Trim() == "99") && (pFeature.get_Value(num4).ToString().Trim() == ""))
                                {
                                    if (this.richTextBoxj.Text != "")
                                    {
                                        this.richTextBoxj.AppendText(string.Concat(new object[] { "\n来自遥感判读相交比率95%以上且未经过核实的变更小班[要素", pFeature.OID, "]导入", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]属性信息" }));
                                    }
                                    else
                                    {
                                        this.richTextBoxj.Text = string.Concat(new object[] { "来自遥感判读相交比率95%以上且未经过核实的变更小班[要素", pFeature.OID, "]导入", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]属性信息" });
                                    }
                                    this.richTextBoxj.Refresh();
                                    this.ReadValue(pFeature, psf);
                                    this.SetAreaXJ(pFeature);
                                    pFeature.Store();
                                }
                                else if ((pFeature.get_Value(index).ToString().Trim() == "99") && (pFeature.get_Value(num4).ToString().Trim() != ""))
                                {
                                    num7 = psf.Fields.FindField("BHYY");
                                    if (psf.get_Value(num7).ToString().Trim() == "")
                                    {
                                        if (this.richTextBoxj.Text != "")
                                        {
                                            this.richTextBoxj.Text = string.Concat(new object[] { this.richTextBoxj.Text, "\n与来自遥感判读的变更小班[要素", pFeature.OID, "]相交的", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]变化原因为空--忽略" });
                                        }
                                        else
                                        {
                                            this.richTextBoxj.Text = string.Concat(new object[] { "与来自遥感判读的变更小班[要素", pFeature.OID, "]相交的", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]变化原因为空--忽略" });
                                        }
                                        this.richTextBoxj.Refresh();
                                        flag = false;
                                        goto Label_53A8;
                                    }
                                    if (pFeature.get_Value(num4).ToString() != "")
                                    {
                                        if ((((int.Parse(pFeature.get_Value(num4).ToString()) >= 20) && (int.Parse(pFeature.get_Value(num4).ToString()) < 30)) && (int.Parse(psf.get_Value(num7).ToString()) >= 1)) && (int.Parse(psf.get_Value(num7).ToString()) < 20))
                                        {
                                            if (this.richTextBoxj.Text != "")
                                            {
                                                this.richTextBoxj.Text = string.Concat(new object[] { this.richTextBoxj.Text, "\n来自遥感判读的相交比率95%以上且变更原因为采伐的变更小班[要素", pFeature.OID, "]导入", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]属性信息" });
                                            }
                                            else
                                            {
                                                this.richTextBoxj.Text = string.Concat(new object[] { "来自遥感判读的相交比率95%以上且变更原因为采伐的变更小班[要素", pFeature.OID, "]导入", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]属性信息" });
                                            }
                                            this.richTextBoxj.Refresh();
                                            this.ReadValue(pFeature, psf);
                                            this.SetAreaXJ(pFeature);
                                        ...
                                        else if ((int.Parse(pFeature.get_Value(num4).ToString()) < 20) && (int.Parse(psf.get_Value(num7).ToString()) < 20))
                                        {
                                            if (this.richTextBoxj.Text != "")
                                            {
                                                this.richTextBoxj.Text = string.Concat(new object[] { this.richTextBoxj.Text, "\n与来自遥感判读的变更小班[要素", pFeature.OID, "]相交比率95%以上", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]因变更原因为造林--忽略" });
                                            }
                                            else
                                            {
                                                this.richTextBoxj.Text = string.Concat(new object[] { "与来自遥感判读的变更小班[要素", pFeature.OID, "]相交比率95%以上", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]因变更原因为造林--忽略" });
                                            }
                                        }
                                        else
                                        {
                                            str = psf.get_Value(psf.Fields.FindField("GXSJ")).ToString();
                                            if (str.Trim() == "")
                                            {
                                                if (this.richTextBoxj.Text != "")
                                                {
                                                    this.richTextBoxj.Text = string.Concat(new object[] { this.richTextBoxj.Text, "\n与变更小班[要素", pFeature.OID, "]相交的", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]更新时间为空--忽略" });
                                                }
                                                else
                                                {
                                                    this.richTextBoxj.Text = string.Concat(new object[] { "与变更小班[要素", pFeature.OID, "]相交的", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]更新时间为空--忽略" });
                                                }
                                                this.richTextBoxj.Refresh();
                                                flag = false;
                                                goto Label_53A8;
                                            }
                                            if (pFeature.get_Value(num5).ToString().Trim() == "")
                                            {
                                                if (this.richTextBoxj.Text != "")
                                                {
                                                    this.richTextBoxj.Text = string.Concat(new object[] { this.richTextBoxj.Text, "\n相交比率95%以上且变更时间为空的变更小班[要素", pFeature.OID, "]导入", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]属性信息" });
                                                }
                                                else
                                                {
                                                    this.richTextBoxj.Text = string.Concat(new object[] { "相交比率95%以上且变更时间为空的变更小班[要素", pFeature.OID, "]导入", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]属性信息" });
                                                }
                                                this.richTextBoxj.Refresh();
                                                this.ReadValue(pFeature, psf);
                                                this.SetAreaXJ(pFeature);
                                            ...
                                            else if (int.Parse(str) > int.Parse(pFeature.get_Value(num5).ToString()))
                                            {
                                                if (this.richTextBoxj.Text != "")
                                                {
                                                    this.richTextBoxj.Text = string.Concat(new object[] { this.richTextBoxj.Text, "\n相交比率95%以上且变更时间较早的变更小班[要素", pFeature.OID, "]导入", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]属性信息" });
                                                }
                                                else
                                                {
                                                    this.richTextBoxj.Text = string.Concat(new object[] { "相交比率95%以上且变更时间较早的变更小班[要素", pFeature.OID, "]导入", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]属性信息" });
                                                }
                                                this.richTextBoxj.Refresh();
                                                this.ReadValue(pFeature, psf);
                                                this.SetAreaXJ(pFeature);
                                            ...
                                            else
                                            {
                                                if (this.richTextBoxj.Text != "")
                                                {
                                                    this.richTextBoxj.Text = string.Concat(new object[] { this.richTextBoxj.Text, "\n相交比率95%以上且变更时间较晚的变更小班[要素", pFeature.OID, "]--忽略" });
                                                }
                                                else
                                                {
                                                    this.richTextBoxj.Text = "相交比率95%以上且变更时间较晚的变更小班[要素" + pFeature.OID + "]--忽略";
                                                }
                                                this.richTextBoxj.Refresh();
                                                flag = false;
                                            }
                                    ...
                                    else
                                    {
                                        ...
                                        if (int.Parse(str) > int.Parse(pFeature.get_Value(num5).ToString()))
                                        {
                                            if (this.richTextBoxj.Text != "")
                                            {
                                                this.richTextBoxj.Text = string.Concat(new object[] { this.richTextBoxj.Text, "\n相交比率95%以上且变更时间较早的变更小班[要素", pFeature.OID, "]导入", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]属性信息" });
                            ...
                            else if (pFeature.get_Value(index).ToString().Trim() != "99")
                            {
                                num7 = psf.Fields.FindField("BHYY");
                                if (psf.get_Value(num7).ToString().Trim() == "")
                                {
                                    if (this.richTextBoxj.Text != "")
                                    {
                                        this.richTextBoxj.AppendText(string.Concat(new object[] { "\n与变更小班[要素", pFeature.OID, "]相交的", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]变化原因为空--忽略" }));
                                    }
                                    else
                                    {
                                        this.richTextBoxj.Text = string.Concat(new object[] { "与变更小班[要素", pFeature.OID, "]相交的", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]变化原因为空--忽略" });
                                    }
                                    this.richTextBoxj.Refresh();
                                    flag = false;
                                    goto Label_53A8;
                                }
                                if ((((int.Parse(pFeature.get_Value(num4).ToString()) >= 10) && (int.Parse(pFeature.get_Value(num4).ToString()) < 20)) && (int.Parse(psf.get_Value(num7).ToString()) >= 20)) && (int.Parse(psf.get_Value(num7).ToString()) < 30))
                                {
                                    if (this.richTextBoxj.Text != "")
                                    {
                                        this.richTextBoxj.AppendText(string.Concat(new object[] { "\n与变更小班[要素", pFeature.OID, "]变化原因为造林相交的", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]变化原因为采伐--忽略" }));
                                    }
                                    else
                                    {
                                        this.richTextBoxj.Text = string.Concat(new object[] { "与变更小班[要素", pFeature.OID, "]变化原因为造林相交的", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]变化原因为采伐--忽略" });
                                    }
                                    this.richTextBoxj.Refresh();
                                    flag = false;
                                    goto Label_53A8;
                                ...
                                if ((((int.Parse(pFeature.get_Value(num4).ToString()) >= 20) && (int.Parse(pFeature.get_Value(num4).ToString()) < 30)) && (int.Parse(psf.get_Value(num7).ToString()) >= 1)) && (int.Parse(psf.get_Value(num7).ToString()) < 20))
                                {
                                    if (this.richTextBoxj.Text != "")
                                    {
                                        this.richTextBoxj.AppendText(string.Concat(new object[] { "\n相交比率95%以上且变更原因为采伐的变更小班[要素", pFeature.OID, "]替换为造林小班[要素", psf.OID, "]" }));
                                    }
                                    else
                                    {
                                        this.richTextBoxj.Text = string.Concat(new object[] { "相交比率95%以上且变更原因为采伐的变更小班[要素", pFeature.OID, "]替换为造林小班[要素", psf.OID, "]" });
                                    }
                                    this.richTextBoxj.Refresh();
                                    geometry2 = null;
                                    feature4 = this.getHasFeature(plist, psf, list2);
                                    if (feature4 != null)
                                    {
                                        feature = feature4;
                                        feature = this.m_EditLayer.FeatureClass.GetFeature(feature4.OID);
                                    ...
                                    {
                                        feature = this.m_EditLayer.FeatureClass.CreateFeature();
                                        geometry2 = psf.ShapeCopy;
                                        feature.Shape = geometry2;
                                        plist.Add(psf);
                                        list2.Add(feature);
                                    }
                                    this.ReadValue(feature, psf);
                                    this.SetAreaXJ(feature);
                                    feature.set_Value(index, EditTask.KindCode.Substring(2, 2));
                                    pFeature.Delete();
                                    pFeature.Store();
                                ...
                                else
                                {
                                    ...
                                    if (pFeature.get_Value(num5).ToString().Trim() == "")
                                    {
                                        if (this.richTextBoxj.Text != "")
                                        {
                                            this.richTextBoxj.AppendText(string.Concat(new object[] { "\n相交比率95%以上且变更时间为空的变更小班[要素", pFeature.OID, "]替换为", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]" }));
                                        }
                                        else
                                        {
                                            this.richTextBoxj.Text = string.Concat(new object[] { "相交比率95%以上且变更时间为空的变更小班[要素", pFeature.OID, "]替换为", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]" });
                                        }
                                        this.richTextBoxj.Refresh();
                                        geometry2 = null;
                                        feature4 = this.getHasFeature(plist, psf, list2);
                                    ...
                                    else if (int.Parse(str) > int.Parse(pFeature.get_Value(num5).ToString()))
                                    {
                                        if (this.richTextBoxj.Text != "")
                                        {
                                            this.richTextBoxj.AppendText(string.Concat(new object[] { "\n相交比率95%以上且变更时间较早的变更小班[要素", pFeature.OID, "]替换为", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]" }));
                                        }
                                        else
                                        {
                                            this.richTextBoxj.Text = string.Concat(new object[] { "相交比率95%以上且变更时间较早的变更小班[要素", pFeature.OID, "]替换为", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]" });
                                        }
                                        this.richTextBoxj.Refresh();
                                        geometry2 = null;
                                        feature4 = this.getHasFeature(plist, psf, list2);
                                    ...
                                    else if (int.Parse(str) == int.Parse(pFeature.get_Value(num5).ToString()))
                                    {
                                        if (this.richTextBoxj.Text != "")
                                        {
                                            this.richTextBoxj.AppendText("\n相交比率95%以上且变更时间相同的变更小班[要素" + pFeature.OID + "]--忽略");
                                        }
                                        else
                                        {
                                            this.richTextBoxj.Text = "相交比率95%以上且变更时间相同的变更小班[要素" + pFeature.OID + "]--忽略";
                                        }
                                        this.richTextBoxj.Refresh();
                                        flag = false;
                                    }
                                    else
                                    {
                                        if (this.richTextBoxj.Text != "")
                                        {
                                            this.richTextBoxj.AppendText("\n相交比率95%以上且变更时间较早的变更小班[要素" + pFeature.OID + "]--忽略");
                                        }
                                        else
                                        {
                                            this.richTextBoxj.Text = "相交比率95%以上且变更时间较早的变更小班[要素" + pFeature.OID + "]--忽略";
                                        }
                                        this.richTextBoxj.Refresh();
                                        flag = false;
                                    }
                        ...
                        else
                        {
                            ...
                            if (pFeature.get_Value(index).ToString().Trim() == "99")
                            {
                                ...
                                if (pFeature.get_Value(num4).ToString() != "")
                                {
                                    if ((((int.Parse(pFeature.get_Value(num4).ToString()) >= 10) && (int.Parse(pFeature.get_Value(num4).ToString()) < 20)) && (int.Parse(psf.get_Value(num7).ToString()) >= 20)) && (int.Parse(psf.get_Value(num7).ToString()) < 30))
                                    {
                                        if (this.richTextBoxj.Text != "")
                                        {
                                            this.richTextBoxj.AppendText(string.Concat(new object[] { "\n与来自遥感判读的变更小班[要素", pFeature.OID, "]变化原因为造林相交的", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]变化原因为采伐--忽略" }));
                                        }
                                        else
                                        {
                                            this.richTextBoxj.Text = string.Concat(new object[] { "与来自遥感判读的变更小班[要素", pFeature.OID, "]变化原因为造林相交的", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]变化原因为采伐--忽略" });
                                        }
                                        this.richTextBoxj.Refresh();
                                        flag = false;
                                        goto Label_53A8;
                                    ...
                                    if ((((int.Parse(pFeature.get_Value(num4).ToString()) >= 20) && (int.Parse(pFeature.get_Value(num4).ToString()) < 30)) && (int.Parse(psf.get_Value(num7).ToString()) >= 1)) && (int.Parse(psf.get_Value(num7).ToString()) < 20))
                                    {
                                        if (this.richTextBoxj.Text != "")
                                        {
                                            this.richTextBoxj.AppendText("\n来自遥感判读的变化原因为采伐的变更小班[要素" + pFeature.OID + "]裁切相交部分");
                                        }
                                        else
                                        {
                                            this.richTextBoxj.Text = "来自遥感判读的变化原因为采伐的变更小班[要素" + pFeature.OID + "]裁切相交部分";
                                        }
                                        this.richTextBoxj.Refresh();
                                        geometry3 = pFeature.ShapeCopy;
                                        geometry4 = psf.ShapeCopy;
                                        if (geometry3.SpatialReference != geometry4.SpatialReference)
                                        {
                                            geometry3.Project(geometry4.SpatialReference);
                                        }
                                        @operator = geometry3 as ITopologicalOperator2;
                                        @operator.IsKnownSimple_2 = false;
                                        @operator.Simplify();
                                        try
                                        {
                                            geometry5 = @operator.Intersect(geometry4, esriGeometryDimension.esriGeometry2Dimension);
                                            if (!geometry5.IsEmpty)
                                            {
                                                operator2 = geometry5 as ITopologicalOperator2;
                                                operator2.IsKnownSimple_2 = false;
                                                operator2.Simplify();
                                                operator3 = geometry3 as ITopologicalOperator2;
                                                geometry6 = operator3.Difference(geometry5);
                                        ...
                                        if (this.richTextBoxj.Text != "")
                                        {
                                            this.richTextBoxj.AppendText("，导入造林小班[要素" + psf.OID + "]");
                                        }
                                        else
                                        {
                                            this.richTextBoxj.Text = "导入造林小班[要素" + psf.OID + "]";
                                        }
                                        this.richTextBoxj.Refresh();
                                        feature4 = this.getHasFeature(plist, psf, list2);
                                        if (feature4 != null)
                                    ...
                                    else if ((int.Parse(pFeature.get_Value(num4).ToString()) == 10) && (int.Parse(psf.get_Value(num7).ToString()) < 20))
                                    {
                                        if (this.richTextBoxj.Text != "")
                                        {
                                            this.richTextBoxj.AppendText("\n变化原因为造林的变更小班[要素" + pFeature.OID + "]裁切相交部分");
                                        }
                                        else
                                        {
                                            this.richTextBoxj.Text = "变化原因为造林的变更小班[要素" + pFeature.OID + "]裁切相交部分";
                                        }
                                        this.richTextBoxj.Refresh();
                                        geometry3 = pFeature.ShapeCopy;
                                        geometry4 = psf.ShapeCopy;
                                    ...
                                    else if ((pFeature.get_Value(num4).ToString() != "") && (pFeature.get_Value(index).ToString() == "99"))
                                    {
                                        if (this.richTextBoxj.Text != "")
                                        {
                                            this.richTextBoxj.AppendText(string.Concat(new object[] { "\n是遥感变化检测带有变化原因的小班[要素", pFeature.OID, "]，裁切", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]相交部分，导入图形和属性" }));
                                        }
                                        else
                                        {
                                            this.richTextBoxj.Text = string.Concat(new object[] { "是遥感变化检测带有变化原因的小班[要素", pFeature.OID, "]，裁切", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]相交部分，导入图形和属性" });
                                        }
                                        this.richTextBoxj.Refresh();
                                        geometry3 = pFeature.ShapeCopy;
                                        geometry4 = psf.ShapeCopy;
                                        ...
                                        try
                                        {
                                            ...
                                            if (!geometry5.IsEmpty)
                                            {
                                                ...
                                                operator2.IsKnownSimple_2 = false;
                                                operator2.Simplify();
                                                operator3 = geometry4 as ITopologicalOperator2;
                                                geometry6 = operator3.Difference(geometry5);
                                                if (!geometry6.IsEmpty)
                                                {
                                                    if (!flag2)
                                    ...
                                    else
                                    {
                                        ...
                                        if (pFeature.get_Value(num5).ToString().Trim() == "")
                                        {
                                            if (this.richTextBoxj.Text != "")
                                            {
                                                this.richTextBoxj.AppendText("\n变更时间为空的变更小班[要素" + pFeature.OID + "]裁切相交部分");
                                            }
                                            else
                                            {
                                                this.richTextBoxj.Text = "变更时间为空的变更小班[要素" + pFeature.OID + "]裁切相交部分";
                                            }
                                            this.richTextBoxj.Refresh();
                                            geometry3 = pFeature.ShapeCopy;
                                            geometry4 = psf.ShapeCopy;
                                            ...
                                            if (this.richTextBoxj.Text != "")
                                            {
                                                this.richTextBoxj.AppendText(string.Concat(new object[] { "，导入", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]" }));
                                            }
                                            else
                                            {
                                                this.richTextBoxj.Text = string.Concat(new object[] { "导入", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]" });
                                            }
                                            this.richTextBoxj.Refresh();
                                            feature4 = this.getHasFeature(plist, psf, list2);
                                            if (feature4 != null)
                                        ...
                                        else if (int.Parse(str) > int.Parse(pFeature.get_Value(num5).ToString()))
                                        {
                                            if (this.richTextBoxj.Text != "")
                                            {
                                                this.richTextBoxj.AppendText("\n变更时间较早的变更小班[要素" + pFeature.OID + "]裁切相交部分");
                                            }
                                            else
                                            {
                                                this.richTextBoxj.Text = "变更时间较早的变更小班[要素" + pFeature.OID + "]裁切相交部分";
                                            }
                                            this.richTextBoxj.Refresh();
                                            geometry3 = pFeature.ShapeCopy;
                                            geometry4 = psf.ShapeCopy;
                                        ...
                                        else if (int.Parse(str) == int.Parse(pFeature.get_Value(num5).ToString()))
                                        {
                                            if (this.richTextBoxj.Text != "")
                                            {
                                                this.richTextBoxj.AppendText("\n变更时间相同的变更小班[要素" + pFeature.OID + "]--忽略");
                                            }
                                            else
                                            {
                                                this.richTextBoxj.Text = "变更时间相同的变更小班[要素" + pFeature.OID + "]--忽略";
                                            }
                                            this.richTextBoxj.Refresh();
                                            flag = false;
                                        }
                                        else
                                        {
                                            if (this.richTextBoxj.Text != "")
                                            {
                                                this.richTextBoxj.AppendText(string.Concat(new object[] { "\n变更时间较晚的变更小班[要素", pFeature.OID, "]，裁切", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]相交部分，导入图形和属性" }));
                                            }
                                            else
                                            {
                                                this.richTextBoxj.Text = string.Concat(new object[] { "变更时间较晚的变更小班[要素", pFeature.OID, "]，裁切", this.m_UnderLayer.Name, "小班[要素", psf.OID, "]相交部分，导入图形和属性" });
                                            }
                                            this.richTextBoxj.Refresh();
                                            geometry3 = pFeature.ShapeCopy;
                                            geometry4 = psf.ShapeCopy;
                                ...
                                else
                                {
                                    ...
                                    if (pFeature.get_Value(num5).ToString() == "")
                                    {
                                        if (this.richTextBoxj.Text != "")
                                        {
                                            this.richTextBoxj.AppendText("\n变更时间为空的变更小班[要素" + pFeature.OID + "]裁切相交部分");
                            ...
                            else if (pFeature.get_Value(index).ToString().Trim() != "99")
                            {
                                ...
                                if ((((int.Parse(pFeature.get_Value(num4).ToString()) >= 20) && (int.Parse(pFeature.get_Value(num4).ToString()) < 30)) && (int.Parse(psf.get_Value(num7).ToString()) >= 1)) && (int.Parse(psf.get_Value(num7).ToString()) < 20))
                                {
                                    if (this.richTextBoxj.Text != "")
                                    {
                                        this.richTextBoxj.AppendText("\n变化原因为采伐的变更小班[要素" + pFeature.OID + "]裁切相交部分");
                                    }
                                    else
                                    {
                                        this.richTextBoxj.Text = "变化原因为采伐的变更小班[要素" + pFeature.OID + "]裁切相交部分";
                                    }
                                    this.richTextBoxj.Refresh();
                                    geometry3 = pFeature.ShapeCopy;
                                    geometry4 = psf.ShapeCopy;
                                    ...
                                    else
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            if (!geometry5.IsEmpty)
                                            {
                                                ...
                                                operator2.IsKnownSimple_2 = false;
                                                operator2.Simplify();
                                                geometry6 = (geometry4 as ITopologicalOperator2).Difference(geometry5);
                                                if (!geometry6.IsEmpty)
                                                {
                                                    if (!flag2)
                                                    {
                ...
                Label_53A8:
                    ...
                    if (!pFeature.Shape.IsEmpty)
                    {
                        geometry7 = pFeature.ShapeCopy;
                        if (geometry7.SpatialReference != this.mHookHelper.FocusMap.SpatialReference)
                        {
                            geometry7.Project(this.mHookHelper.FocusMap.SpatialReference);
                            geometry7.SpatialReference = this.mHookHelper.FocusMap.SpatialReference;
                        }
                        GISFunFactory.FlashFun.FlashFeature(this.mHookHelper.FocusMap, pFeature, 300L, true);
                        this.mHookHelper.ActiveView.PartialRefresh(esriViewDrawPhase.esriViewForeground, this.m_EditLayer, geometry7.Envelope);
                        this.mHookHelper.ActiveView.PartialRefresh(esriViewDrawPhase.esriViewBackground, this.m_EditLayer, geometry7.Envelope);
                    }
                    if ((feature != null) && !feature.Shape.IsEmpty)
                    {
                        geometry7 = feature.ShapeCopy;
                        if (geometry7.SpatialReference != this.mHookHelper.FocusMap.SpatialReference)
                        {
                            geometry7.Project(this.mHookHelper.FocusMap.SpatialReference);
                            geometry7.SpatialReference = this.mHookHelper.FocusMap.SpatialReference;
        ...
        private void DoCheckLayer(IFeatureLayer pLayer)
        {
            try
            {
                ...
                {
                    this.labelChkprogress.Text = "检查不在区划范围内的图斑...";
                    if (featureClass == null)
                    {
                        this.richTextChk.Text = this.richTextChk.Text + "--失败";
                    }
                    IEnvelope extent = ((IGeoDataset) featureClass).Extent;
                    if (extent.IsEmpty)
                    {
                        this.richTextChk.Text = string.Concat(new object[] { this.richTextChk.Text, "\n", pLayer.Name, "图层有", 0, "个不在区划范围内的班块" });
                        this.richTextChk.Refresh();
                    }
                    else
                    {
                        ...
                        if (this.richTextChk.Text != "")
                        {
                            this.richTextChk.Text = string.Concat(new object[] { this.richTextChk.Text, "\n", pLayer.Name, "图层有", num2, "个不在区划范围内的班块" });
                        }
                        else
                        {
                            this.richTextChk.Text = string.Concat(new object[] { pLayer.Name, "图层有", num2, "个不在区划范围内的班块" });
                        }
                        this.richTextChk.Refresh();
                        if (num2 > 0)
                        {
                ...
                queryFilter.WhereClause = "BHYY='' or GXSJ=''";
                int num3 = featureClass.FeatureCount(queryFilter);
                if (this.richTextChk.Text != "")
                {
                    this.richTextChk.Text = string.Concat(new object[] { this.richTextChk.Text, "\n", pLayer.Name, "图层有", num3, "个变化原因或更新时间未填写的班块" });
                }
                else
                {
                    this.richTextChk.Text = string.Concat(new object[] { pLayer.Name, "图层有", num3, "个变化原因或更新时间未填写的班块" });
                }
                this.richTextChk.Refresh();
                if (num3 > 0)
                {
        ...
        private void DoInput2(IWorkspaceEdit pWorkspaceEdit, IFeatureClass pSFeatureClass, IFeatureClass pTFeatureClass)
        {
            try
            {
                ...
                int num3 = int.Parse(EditTask.TaskYear) - 1;
                IFeatureClass pSubFClass = this.mFeatureWorkspace.OpenFeatureClass(configValue + "_" + num3.ToString());
                pWorkspaceEdit.StartEditing(false);
                int num = 0;
                for (int i = 0; i < this.tList2.Nodes.Count; i++)
                {
                    ...
                    if (this.tList2.Nodes[i].Checked && (this.tList2.Nodes[i].Tag != null))
                    {
                        ...
                        if (pFeature != null)
                        {
                            ...
                            bool flag = UpdateSub.ImportZTFeature(EditTask.KindCode.Substring(2, 2), pFeature, this.m_EditLayer.FeatureClass, pSubFClass, this.mHookHelper.FocusMap.SpatialReference);
                            GC.Collect();
                            num++;
                            if (num == 100)
                            {
        ...
        private void GetXBInfo()
        {
            try
            {
                ...
                queryFilter.WhereClause = configValue + "<>'0'";
                ...
                if (num3 > 0)
                {
                    ...
                    num3 = this.m_EditLayer.FeatureClass.FeatureCount(queryFilter);
                    this.labelXBInfo.Text = string.Concat(new object[] { this.labelXBInfo.Text, "(其中变化原因为植树造林的", num3, "个" });
                    queryFilter.WhereClause = name + " like '2%' and " + configValue + "<>'0'";
                    num3 = this.m_EditLayer.FeatureClass.FeatureCount(queryFilter);
                    this.labelXBInfo.Text = string.Concat(new object[] { this.labelXBInfo.Text, "，森林采伐的", num3, "个" });
                    queryFilter.WhereClause = name + " like '3%' and " + configValue + "<>'0'";
                    num3 = this.m_EditLayer.FeatureClass.FeatureCount(queryFilter);
                    this.labelXBInfo.Text = string.Concat(new object[] { this.labelXBInfo.Text, "，规划调整的", num3, "个" });
                    queryFilter.WhereClause = name + " like '4%' and " + configValue + "<>'0'";
                    num3 = this.m_EditLayer.FeatureClass.FeatureCount(queryFilter);
                    this.labelXBInfo.Text = string.Concat(new object[] { this.labelXBInfo.Text, "，占用征收的", num3, "个" });
                    queryFilter.WhereClause = name + " like '5%' and " + configValue + "<>'0'and " + configValue + "<>'0'";
                    num3 = this.m_EditLayer.FeatureClass.FeatureCount(queryFilter);
                    this.labelXBInfo.Text = string.Concat(new object[] { this.labelXBInfo.Text, "，毁林开荒的", num3, "个" });
                    queryFilter.WhereClause = name + " = '71' and " + configValue + "<>'0'";
                    num3 = this.m_EditLayer.FeatureClass.FeatureCount(queryFilter);
                    this.labelXBInfo.Text = string.Concat(new object[] { this.labelXBInfo.Text, "，火灾的", num3, "个" });
                    queryFilter.WhereClause = "(" + name + " = '70' or " + name + " = '72' or " + name + " = '73') and " + configValue + "<>'0'";
                    num3 = this.m_EditLayer.FeatureClass.FeatureCount(queryFilter);
                    this.labelXBInfo.Text = string.Concat(new object[] { this.labelXBInfo.Text, "，灾害因素的", num3, "个" });
                    queryFilter.WhereClause = name + " like '7%' and " + configValue + "<>'0'";
                    num3 = this.m_EditLayer.FeatureClass.FeatureCount(queryFilter);
                    this.labelXBInfo.Text = string.Concat(new object[] { this.labelXBInfo.Text, "，自然因素的", num3, "个" });
                    queryFilter.WhereClause = name + " like '9%' and " + configValue + "<>'0'";
                    num3 = this.m_EditLayer.FeatureClass.FeatureCount(queryFilter);
                    this.labelXBInfo.Text = string.Concat(new object[] { this.labelXBInfo.Text, "，调查因素的", num3, "个" });
                    this.labelXBInfo.Text = this.labelXBInfo.Text + ")";
                }
                queryFilter.WhereClause = name + " is not null and " + name + "<>'' and (" + configValue + " is null or " + configValue + " ='')";
                num3 = this.m_EditLayer.FeatureClass.FeatureCount(queryFilter);
                this.labelXBInfo.Text = string.Concat(new object[] { this.labelXBInfo.Text, "\r\n非遥感判读导入班块", num3, "个" });
                queryFilter.WhereClause = name + " like '1%' and " + name + "<>'' and (" + configValue + " is null or " + configValue + " ='')";
                num3 = this.m_EditLayer.FeatureClass.FeatureCount(queryFilter);
                this.labelXBInfo.Text = string.Concat(new object[] { this.labelXBInfo.Text, "(其中变化原因为植树造林的", num3, "个" });
                queryFilter.WhereClause = name + " like '2%' and " + name + "<>'' and (" + configValue + " is null or " + configValue + " ='')";
                num3 = this.m_EditLayer.FeatureClass.FeatureCount(queryFilter);
                this.labelXBInfo.Text = string.Concat(new object[] { this.labelXBInfo.Text, "，森林采伐的", num3, "个" });
                queryFilter.WhereClause = name + " like '3%' and " + name + "<>'' and (" + configValue + " is null or " + configValue + " ='')";
                num3 = this.m_EditLayer.FeatureClass.FeatureCount(queryFilter);
                this.labelXBInfo.Text = string.Concat(new object[] { this.labelXBInfo.Text, "，规划调整的", num3, "个" });
                queryFilter.WhereClause = name + " like '4%' and " + name + "<>'' and (" + configValue + " is null or " + configValue + " ='')";
                num3 = this.m_EditLayer.FeatureClass.FeatureCount(queryFilter);
                this.labelXBInfo.Text = string.Concat(new object[] { this.labelXBInfo.Text, "，占用征收的", num3, "个" });
                queryFilter.WhereClause = name + " like '5%' and " + name + "<>'' and (" + configValue + " is null or " + configValue + " ='')";
                num3 = this.m_EditLayer.FeatureClass.FeatureCount(queryFilter);
                this.labelXBInfo.Text = string.Concat(new object[] { this.labelXBInfo.Text, "，毁林开荒的", num3, "个" });
                queryFilter.WhereClause = name + " = '71' and " + name + "<>'' and (" + configValue + " is null or " + configValue + " ='')";
                num3 = this.m_EditLayer.FeatureClass.FeatureCount(queryFilter);
                this.labelXBInfo.Text = string.Concat(new object[] { this.labelXBInfo.Text, "，火灾的", num3, "个" });
                queryFilter.WhereClause = "(" + name + " = '70' or " + name + " = '72' or " + name + " = '73' ) and " + name + "<>'' and (" + configValue + " is null or " + configValue + " ='')";
                num3 = this.m_EditLayer.FeatureClass.FeatureCount(queryFilter);
                this.labelXBInfo.Text = string.Concat(new object[] { this.labelXBInfo.Text, "，灾害因素的", num3, "个" });
                queryFilter.WhereClause = name + " like '7%' and " + name + "<>'' and (" + configValue + " is null or " + configValue + " ='')";
                num3 = this.m_EditLayer.FeatureClass.FeatureCount(queryFilter);
                this.labelXBInfo.Text = string.Concat(new object[] { this.labelXBInfo.Text, "，自然因素的", num3, "个" });
                queryFilter.WhereClause = name + " like '9%' and " + name + "<>'' and (" + configValue + " is null or " + configValue + " ='')";
                num3 = this.m_EditLayer.FeatureClass.FeatureCount(queryFilter);
                this.labelXBInfo.Text = string.Concat(new object[] { this.labelXBInfo.Text, "，调查因素的", num3, "个" });
                this.labelXBInfo.Text = this.labelXBInfo.Text + ")";
        ...
        private void InitializeComponent()
        {
            ...
            ComponentResourceManager resources = new ComponentResourceManager(typeof(UserControlXBSet2));
            ...
            this.panel6.Location = new System.Drawing.Point(4, 0x87);
            this.panel6.Name = "panel6";
            this.panel6.Padding = new Padding(0, 4, 0, 4);
//            this.panel6.RightToLeft = RightToLeft.Yes;
            this.panel6.Size = new Size(0x126, 0x20);
            ...
            this.simpleButtonFinish.Location = new System.Drawing.Point(0x66, 4);
            ...
            this.panel7.Location = new System.Drawing.Point(0xa2, 4);
            ...
            this.simpleButtonOK.Location = new System.Drawing.Point(0xa8, 4);
            ...
            this.panel1.Location = new System.Drawing.Point(0xe4, 4);
            ...
            this.simpleButtonCancel.Location = new System.Drawing.Point(0xea, 4);
            ...
            this.tListKind.Location = new System.Drawing.Point(4, 0x33);
            ...
            nodeData4[0] = "林地征占用";
            this.tListKind.AppendNode(nodeData4, -1, -1, -1, 0x2a);
            object[] nodeData5 = new object[6];
            nodeData5[0] = "自然灾害";
            this.tListKind.AppendNode(nodeData5, -1, -1, -1, 0x2d);
            object[] nodeData6 = new object[6];
            nodeData6[0] = "林业案件";
            this.tListKind.AppendNode(nodeData6, -1, -1, -1, 0x12);
            object[] nodeData7 = new object[6];
            nodeData7[0] = "冲突专题数据";
            nodeData7[2] = "";
            this.tListKind.AppendNode(nodeData7, -1, -1, -1, 0x20);
            ...
            this.tListKind.Size = new Size(0x126, 0x54);
            ...
            this.panelIDList.Location = new System.Drawing.Point(4, 0xa7);
            this.panelIDList.Name = "panelIDList";
            this.panelIDList.Padding = new Padding(0, 4, 0, 2);
            this.panelIDList.Size = new Size(0x126, 0xd5);
            ...
            this.xtraTabControl1.Size = new Size(0x126, 0xab);
            ...
            this.xtraTabPage1.Size = new Size(0x121, 0x90);
            item.Text = "与造林专题相交班块";
            tip.Items.Add(item);
            ...
            this.panel9.Size = new Size(0x119, 30);
            ...
            this.simpleButtonBack2.Location = new System.Drawing.Point(0x4b, 4);
            this.simpleButtonBack2.Name = "simpleButtonBack2";
            this.simpleButtonBack2.Size = new Size(60, 0x1a);
            ...
            this.panel12.Location = new System.Drawing.Point(0x87, 4);
            this.panel12.Name = "panel12";
            this.panel12.Size = new Size(6, 0x1a);
            ...
            this.simpleButtonAuto.Location = new System.Drawing.Point(0x8d, 4);
            this.simpleButtonAuto.Name = "simpleButtonAuto";
            this.simpleButtonAuto.Size = new Size(0x4a, 0x1a);
            ...
            this.panel10.Location = new System.Drawing.Point(0xd7, 4);
            this.panel10.Name = "panel10";
            this.panel10.Size = new Size(6, 0x1a);
            ...
            this.simpleButton3.Location = new System.Drawing.Point(0xdd, 4);
            this.simpleButton3.Name = "simpleButton3";
            this.simpleButton3.Size = new Size(60, 0x1a);
            ...
            this.tList.Size = new Size(0x119, 0x88);
            ...
            this.tListCol5.Caption = "造林小班";
            this.tListCol5.FieldName = "造林小班";
            ...
            this.panelLog2.Size = new Size(0x119, 0x88);
            ...
            this.panelControl2.Size = new Size(0x119, 0x68);
...
//            this.richTextBoxj.BorderStyle = BorderStyle.None;
            ...
            this.richTextBoxj.Size = new Size(0x115, 100);
            ...
            this.panel13.Size = new Size(0x119, 0x1a);
            ...
            this.labelProgressj.Size = new Size(0x119, 0x16);
            ...
            this.xtraTabPage2.Size = new Size(0x125, 0x94);
            item2.Text = "造林专题独有班块";
            ...
            this.panelLog.Size = new Size(0x11d, 110);
            ...
            this.panelControl1.Size = new Size(0x11d, 0x4e);
...
//            this.richTextBox.BorderStyle = BorderStyle.None;
            ...
            this.richTextBox.Size = new Size(0x119, 0x4a);
            ...
            this.panel2.Size = new Size(0x11d, 0x1a);
            ...
            this.labelprogress.Size = new Size(0x11d, 0x16);
            ...
            this.tList2.Size = new Size(0x11d, 110);
            ...
            this.panel3.Location = new System.Drawing.Point(4, 0x72);
            this.panel3.Name = "panel3";
            this.panel3.Padding = new Padding(0, 4, 0, 0);
            this.panel3.Size = new Size(0x11d, 30);
            ...
            this.simpleButtonBack.Location = new System.Drawing.Point(0x21, 4);
            ...
            this.simpleButtonInput.Location = new System.Drawing.Point(0x5d, 4);
            this.simpleButtonInput.Name = "simpleButtonInput";
            this.simpleButtonInput.Size = new Size(60, 0x1a);
            ...
            this.panel5.Location = new System.Drawing.Point(0x99, 4);
            this.panel5.Name = "panel5";
            this.panel5.Size = new Size(6, 0x1a);
            ...
            this.simpleButtonSelAll.Location = new System.Drawing.Point(0x9f, 4);
            this.simpleButtonSelAll.Name = "simpleButtonSelAll";
            this.simpleButtonSelAll.Size = new Size(60, 0x1a);
            ...
            this.panel4.Location = new System.Drawing.Point(0xdb, 4);
            ...
            this.simpleButton2.Location = new System.Drawing.Point(0xe1, 4);
            this.simpleButton2.Name = "simpleButton2";
            this.simpleButton2.Size = new Size(60, 0x1a);
            ...
            this.xtraTabPage3.Size = new Size(0x125, 0x94);
            item3.Text = "专题间冲突班块";
            ...
            this.tList3.Size = new Size(0x125, 110);
            ...
            this.panel8.Size = new Size(0x125, 0x26);
            ...
            this.labelprogress2.Size = new Size(0xd5, 0x1a);
            ...
            this.labelZTCount.Size = new Size(0x126, 0x12);
            ...
            this.labelinfo.Location = new System.Drawing.Point(0, 0xc1);
            this.labelinfo.Name = "labelinfo";
            this.labelinfo.Size = new Size(0x126, 0x12);
            ...
            this.labelIdentify.Size = new Size(0x12e, 0x1a);
            ...
            this.splitterControl1.Location = new System.Drawing.Point(4, 0xa7);
            this.splitterControl1.Name = "splitterControl1";
            this.splitterControl1.Size = new Size(0x126, 6);
            ...
            this.panelInfo.Size = new Size(0x12e, 0x5e);
            ...
            this.labelXBInfo.Size = new Size(0x12e, 0x56);
            ...
            this.groupControlCheck.Location = new System.Drawing.Point(4, 0x1f8);
            this.groupControlCheck.Name = "groupControlCheck";
            this.groupControlCheck.Padding = new Padding(2);
            this.groupControlCheck.Size = new Size(0x12e, 300);
            ...
            this.panelLogChk.Location = new System.Drawing.Point(4, 170);
            this.panelLogChk.Name = "panelLogChk";
            this.panelLogChk.Padding = new Padding(0, 6, 0, 0);
            this.panelLogChk.Size = new Size(0x126, 0x7e);
            ...
            this.panelControl3.Size = new Size(0x126, 0x5e);
...
//            this.richTextChk.BorderStyle = BorderStyle.None;
            ...
            this.richTextChk.Size = new Size(290, 90);
            ...
            this.panel16.Size = new Size(0x126, 0x1a);
            ...
            this.labelChkprogress.Size = new Size(0x126, 0x16);
            ...
            this.tListKind2.Location = new System.Drawing.Point(4, 0x33);
            this.tListKind2.Name = "tListKind2";
            this.tListKind2.BeginUnboundLoad();
           object[] nodeData10 = new object[5];
           nodeData10[0] = "造林";
           nodeData10[1] = "未通过";
           this.tListKind2.AppendNode(nodeData10, -1, 0, 0, 0x24);
           object[] nodeData11 = new object[5];
           nodeData11[0] = "采伐";
           nodeData11[1] = "未通过";
           this.tListKind2.AppendNode(nodeData11, -1, -1, -1, 0x25);
            object[] nodeData12 = new object[5];
            nodeData12[0] = "森林火灾";
            nodeData12[1] = "未通过";
            this.tListKind2.AppendNode(nodeData, -1, -1, -1, 0x2b);
            object[] nodeData13 = new object[5];
            nodeData13[0] = "林地征占用";
            this.tListKind2.AppendNode(nodeData13, -1, -1, -1, 0x2a);
            object[] nodeData14 = new object[5];
            nodeData14[0] = "自然灾害";
            this.tListKind2.AppendNode(nodeData14, -1, -1, -1, 0x2d);
            object[] nodeData15 = new object[5];
            nodeData15[0] = "林业案件";
            this.tListKind2.AppendNode(nodeData, -1, -1, -1, 0x12);
            ...
            this.tListKind2.Size = new Size(0x126, 0x77);
            ...
            this.label2.Size = new Size(0x126, 0x1a);
            ...
            this.panel11.Location = new System.Drawing.Point(4, 170);
            this.panel11.Name = "panel11";
            this.panel11.Padding = new Padding(0, 4, 0, 0);
            this.panel11.Size = new Size(0x126, 30);
            ...
            this.simpleButtonCheckAll.Location = new System.Drawing.Point(0x90, 4);
            ...
            this.panel14.Location = new System.Drawing.Point(0xd8, 4);
            ...
            this.simpleButtonCheck.Location = new System.Drawing.Point(0xde, 4);
            ...
            this.groupControlCombine.Location = new System.Drawing.Point(4, 120);
            this.groupControlCombine.Name = "groupControlCombine";
            this.groupControlCombine.Padding = new Padding(2);
            this.groupControlCombine.Size = new Size(0x12e, 0x180);
            ...
            this.label1.Size = new Size(0x126, 0x1a);
...
//            base.AutoScaleMode = AutoScaleMode.Font;
            ...
            base.Name = "UserControlXBSet2";
            base.Padding = new Padding(4, 0, 4, 0);
            base.Size = new Size(310, 0x328);
        ...
        private void ReadValue(IFeature pf, IFeature psf)
        {
            try
            {
                int num4;
